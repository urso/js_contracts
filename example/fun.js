 ...
 * this function.
 */
Function.prototype.map = function(fn){ return $compose(fn, this); };
    ...
    if(arguments.length == 1){
        ...
        for(var i=0; i < names.length && cur; i++){
            if(cur instanceof Array) {
                cur = cur.map(function(c){ return c[names[i]]; })
                         .filter($not(isNull));
            } else {
                cur = cur[names[i]];
            }
...
function zip(fn) {
    var as = $A(arguments);
    as.splice(0,1);
    var len = as.foldl(function(len, arr){ return Math.min(len, arr.length); }, 
                       as[0].length);
    var arr = new Array(len);

    for(var i=0; i < len; i++) {
        arr[i] = fn.apply(this, as.map(function(a){ return a[i]; }));
    }
    return arr;
}
 ...
 */
function $setText(obj){ return function(txt){ obj.set('text', txt); }; } 

/**
 * $getText returns inner text of a given DOM element
 */
function $getText(obj){ return obj.innerText || obj.textContent; };

/**
 * $setText sets inner html of a given DOM element (needs mootools)
 */
function $setHTML(obj){ return function(txt){ obj.set('html', txt); }; }
